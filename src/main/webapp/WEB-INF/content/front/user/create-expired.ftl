<title>ユーザー登録</title>

<link href="${assets('user.css', 'css')}" media="screen, print" rel="stylesheet" type="text/css" >

<section class="clearfix">
  <div class="clmn-left-main">
    <h1 class="icon-plus c-pt">ユーザー登録(無料)</h1>
    <p class="c-ng-label bold">登録出来ませんでした。</p>
    <p class="uc-expired-d c-err-d">
有効期限切れ、または既に登録されている可能性があります。<br/>
    <span class="icon-circle-arrow-right"><a href="${url('/user/send-password')}">パスワードを忘れた方はこちら</a></span>
    </p>
  </div>
</section>