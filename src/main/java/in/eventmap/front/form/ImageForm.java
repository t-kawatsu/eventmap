...
public class ImageForm extends AbstractForm {
	...
	private static final String[] ALLOW_CONTENT_TYPES = { "image/jpeg",
			"image/jpg", "image/png", "image/x-png", "image/gif" };
	...
	public boolean validate(ActionSupport as) {
		if (null == getFile()) {
			as.addFieldError("imageForm.file",
					as.getText("invalidate.fileUpload"));
		}
		if (!ArrayUtils.contains(ALLOW_CONTENT_TYPES, getFileContentType())) {
			as.addFieldError("imageForm.fileContentType",
					as.getText("invalidate.fileContentType"));
		}
