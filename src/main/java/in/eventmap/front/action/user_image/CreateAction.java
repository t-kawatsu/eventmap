...
public class CreateAction extends AbstractAction {
	...
	@Action(value = "user-image/create-ajax", results = {
	...
	public String createAjaxAction() throws Exception {
		...
		user = userImageService.create(user, userImageForm.getImageMetas());
		setCurrentUser(user);
		return SUCCESS;
	}

	@Action(value = "user-image/upload-ajax", results = {
	...
	public String uplodAjaxAction() throws Exception {
		...
		User user = getCurrentUser();
		String fileId = userImageService.storeTmpImage(imageForm.getFile(),
				user.getId());
