...
public class CreateAction extends AbstractTwAction {
	...
	@Action(value = "tw-user/create", results = {
	...
	@Action(value = "tw-user/create-complete", results = {
			@Result(name = "input", location = "user/create.ftl"),
	...
	public String completeAction() throws Exception {
		...
		user = twUserService.createUser(userForm, LanguageCode.JA,
				request.getHeader("User-Agent"));
		try {
			userImageService.createFromUrl(user, new URL(twitter
					.verifyCredentials().getBiggerProfileImageURLHttps()));
		} catch (Exception e) {
		}
		setCurrentUser(user);
