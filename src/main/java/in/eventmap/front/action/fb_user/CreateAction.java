...
public class CreateAction extends AbstractFbAction {
	...
	public String completeAction() throws Exception {
		...
		in.eventmap.common.entity.User user = userDao.findByFbIdAndState(
				facebookUser.getId(), UserStatus.LIVE);
		if (user != null) {
			// already exists.
			setCurrentUser(user);
			return "exists";
		...
		user = fbUserService.createUser(userForm, LanguageCode.JA,
