...
public class MergeAction extends AbstractFbAction {
	...
	@Action(value = "fb-user/merge-complete", results = { @Result(name = "success", location = "user/update", type = "redirect") })
	public String completeAction() throws Exception {
		in.eventmap.common.entity.User user = getCurrentUser();
		if (user == null) {
			throw new Exception();
		}
		if (user.hasAccountType(UserAccountType.FACEBOOK)) {
		...
		try {
			facebookClient = getAccessTokenObteinedFacebookClient(
					"/fb-user/merge-complete", code);
		...
		User _user = userDao.findByFbIdAndState(facebookUser.getId(),
				UserStatus.LIVE);
		if (_user != null) {
			// 既に存在する
