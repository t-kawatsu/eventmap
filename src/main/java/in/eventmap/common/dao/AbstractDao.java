...
import in.eventmap.common.entity.AbstractEntity;
import in.eventmap.common.hibernate.Association;
...
abstract public class AbstractDao<T> {
	...
	public int detectCountValue(Criteria c) {
		...
		return ret != null && 0 < ret.size() ? ((Long) ret.get(0)).intValue()
				: 0;
	}
	...
	@Transactional
	public <LC> int count(List<Criterion> criterions,
			List<Association> associations) {
		List<Long> ret = find(Long.class, criterions, Projections
				.projectionList().add(Projections.rowCount()), associations,
				null, null, null);
		return ret != null && 0 < ret.size() ? ((Long) ret.get(0)).intValue()
				: 0;
	}
	...
	@Transactional
	public <LC> List<LC> find(Class<LC> clazz, List<Criterion> criterions,
			ProjectionList projectionList, List<Association> associations,
		...
		if (associations != null) {
			for (Association row : associations) {
				if (row.getWithClause() != null) {
					criteria.createCriteria(row.getAssociationPath(),
							row.getAlias(), row.getJoinType(),
							row.getWithClause());
