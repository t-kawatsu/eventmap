...
import in.eventmap.common.hibernate.type.UserAccountTypeUserType;
...
public class UserDao extends AbstractDao<User> {
	...
	public User findByFbIdAndState(String fbId, UserStatus status) {
		...
		return (User) getSession()
				...
				.setString("fbId", fbId).setParameter("status", status)
				.uniqueResult();
	}
	...
	public User findByTwIdAndState(Long twId, UserStatus status) {
		...
		return (User) getSession()
				...
				.setLong("twId", twId).setParameter("status", status)
				.uniqueResult();
	}
	...
	public int countByMailAddressAndStatusAndAccountType(
		...
		return detectCountValue(getSession()
				.createQuery(sb.toString())
				.setString("mailAddress", mailAddress)
				.setParameter("status", status)
				.setParameter("accountTypes", accountType,
						new CustomType(new UserAccountTypeUserType(), null)));
	}
	...
	public User findByMailAddressAndPasswordAndStatus(String mailAddress,
			String plainPassword, UserStatus status) throws HibernateException,
			Exception {
		if (mailAddress == null || plainPassword == null) {
			return null;
		}
